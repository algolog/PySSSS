...
def doSplit(N, K):
    ...
    for i in range(0, len(BIP39)):
        ...
        data.append(i & 0xFF)
        secrets = zip(
            PySSSS.encodeByte(GF256.QR, data[0], N, K),
            PySSSS.encodeByte(GF256.QR, data[1], N, K),
        )
...
def doRecover(words):
    ...
    print(
        "[%d]  %s  %d/%d"
        % (results[0] + 1, BIP39[results[0]], firstunique, len(splits))
    )
